chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
  ...
  // Truncate the data to avoid token overflow
  const maxItems = 20; // Limiting the number of items to 20
      ...
      try {
        const data = await fetchWithRetry('https://api.openai.com/v1/completions', {
          ...
          body: JSON.stringify({
            ...
            prompt: prompt,
            max_tokens: 300, // 75 words =~ 100 token
            temperature: 0.5, // for variation in responses
            top_p: 1,
            frequency_penalty: 0,
            presence_penalty: 0
          })
